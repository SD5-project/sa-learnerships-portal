...
beforeEach(() => {
    jest.clearAllMocks();
    // Default: notifications add always succeeds
...
describe("US-07: PATCH /api/admin/listings/:id/approve", () => {
    ...
    const setupProviderDoc = () => {
        // Second doc get = provider profile
        mockDocGet
            .mockResolvedValueOnce({ exists: true, data: () => ({ title: "Test Listing", providerID: "provider-uid" }) }) // listing
            .mockResolvedValueOnce({ exists: true, data: () => ({ email: "prov@test.com", organization: "Corp SA", firstname: "Sam" }) }); // provider
    };
    ...
    test("✅ Admin can approve a pending listing", async () => {
        ...
        expect(res.status).toBe(200);
        expect(res.body.message).toBe("Listing approved");
...
describe("US-07: PATCH /api/admin/listings/:id/remove", () => {
    ...
    const setupForRemove = () => {
        mockDocGet
            .mockResolvedValueOnce({ exists: true, data: () => ({ title: "Remove Me", providerID: "provider-uid" }) })
            .mockResolvedValueOnce({ exists: true, data: () => ({ email: "prov@test.com", organization: "Corp", firstname: "Sam" }) });
    };
    ...
    test("✅ Admin can remove a listing", async () => {
        ...
        expect(mockDocUpdate).toHaveBeenCalledWith("Opportunities", "l1",
            expect.objectContaining({ status: "removed", removalReason: "Fraudulent posting" })
        );
    });
...
describe("US-07: /api/listings only returns approved listings", () => {
    ...
    test("✅ Applicant only sees approved listings", async () => {
        mockVerifyIdToken.mockResolvedValue({ uid: "applicant-uid", role: "applicant" });
        mockWhereGet.mockResolvedValue({
            forEach: (cb) => {
                cb({ id: "l1", data: () => ({ title: "Approved", stipend: 3000, location: "JHB", company: "Corp", type: "internship", status: "approved" }) });
            }
        });
    ...
    test("❌ Pending listings are NOT visible to applicants", async () => {
        mockVerifyIdToken.mockResolvedValue({ uid: "applicant-uid", role: "applicant" });
        // The where clause for 'approved' returns empty (pending ones excluded at DB level)
        mockWhereGet.mockResolvedValue({ forEach: () => {} });
...
describe("US-08: GET /api/admin/users", () => {
    ...
    test("✅ Pagination works correctly", async () => {
        ...
        const bigSnapshot = {
            forEach: (cb) => {
                for (let i = 0; i < 25; i++) {
                    cb({ id: `u${i}`, data: () => ({ firstname: `User${i}`, email: `u${i}@t.com`, role: "applicant", status: "active", createdAt: `2024-01-${String(i+1).padStart(2,"0")}` }) });
                }
            }
        };
        mockCollectionGet.mockResolvedValue(bigSnapshot);
...
describe("US-08: PATCH /api/admin/users/:uid/suspend", () => {
    ...
    test("✅ Admin can suspend a user", async () => {
        mockDocGet.mockResolvedValue({ exists: true, data: () => ({ role: "applicant", email: "a@test.com" }) });
    ...
    test("✅ Firebase Auth is disabled on suspend", async () => {
        mockDocGet.mockResolvedValue({ exists: true, data: () => ({ role: "applicant" }) });
        await request(app)
...
describe("US-08: PATCH /api/admin/users/:uid/reactivate", () => {
    ...
    test("✅ Admin can reactivate a suspended user", async () => {
        mockDocGet.mockResolvedValue({ exists: true, data: () => ({ role: "applicant", status: "suspended" }) });
...
describe("US-08: DELETE /api/admin/users/:uid", () => {
    ...
    test("✅ Admin can delete a user", async () => {
        mockDocGet.mockResolvedValue({ exists: true, data: () => ({ role: "applicant", email: "del@test.com" }) });
